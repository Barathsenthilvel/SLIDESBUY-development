...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $screen = $currentRoute ? $currentRoute->getName() : null;
            ...
            // Get store configuration and current currency
            $storeConfig = Storeconfiguration::where('status','1')->orderBy('id','ASC')->first();
            $currentCurrency = null;
            if($storeConfig && $storeConfig->default_currency) {
                $currentCurrency = Currency::find($storeConfig->default_currency);
            }

            $view->with(compact('headMenu'))->with('Category', Category::with('subs')->where('status',1)->where('parent_category_id',0)->get())->with('screen', $screen)->with('frontBanner',Banner::where('status',1)->orderBy('sorting_order','ASC')->get())->with('fronCategory',Category::where('status',1)->where('parent_category_id',0)->get())->with('frontMenu',AdminMenu::with('menuModule')->where('status','1')->orderBy('sort_order','ASC')->get())->with('menuMod',Module::with('getMenu')->where('status','1')->orderBy('sort_order','ASC')->get())->with('StoreConfig', $storeConfig)->with('currentCurrency', $currentCurrency);
